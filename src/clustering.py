...
def get_embeddings(data: List[str], reduce: bool = True, batch_data: bool = True):
    ...
    print("Encoding data...")
    print("Number of strings to encode: ", len(data))
...
def cluster_data(
    ...
    if isinstance(input_data, (str, Path)):
        ...
        processed_data = pl.read_parquet(input_data).unique(subset="literal")

    elif isinstance(input_data, (pl.DataFrame, pd.DataFrame)):

        processed_data = input_data.unique(subset="literal")

    else:
        raise TypeError("input_data can only by a path or a in-memory DataFrame object")

    filtered = [p for p in processed_data["literal"].to_list() if not is_cjk(p)]
    ...
    clean_data = processed_data.filter(pl.col("literal").is_in(filtered))

    ## Saving clusters

    labelled_data = clean_data.with_columns(
        [pl.Series("literal", filtered), pl.Series("label", labels)]
    )
